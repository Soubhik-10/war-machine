...
export function engineeringReport(machine,rules,arenaId='foundry'){
 ...
 if(heat>1)notes.push({level:'warn',category:'Systems',text:`Continuous fire can overheat in about ${Math.max(1,Math.round(100/heat))}s in ${arena.name}. At ${HEAT_CAUTION} heat output starts falling; at ${HEAT_DANGER}, automatic thermal retreat pauses fire until ${HEAT_RECOVERY} heat; at ${HEAT_LIMIT}, weapons lock. Add cooling or reduce continuous weapon heat.`});
 else if(s.heat>0&&s.cooling*env.cooling-s.heat<8)notes.push({level:'warn',category:'Systems',text:`Cooling headroom is only ${(s.cooling*env.cooling-s.heat).toFixed(1)} heat/s. Sustained fire will reach the ${HEAT_DANGER} heat danger band, trigger thermal retreat, and slow weapon cycles.`});
...
export function battleAdvice(battle,side=0){
 ...
 if((v.cooldownTime||0)>1)notes.push(`Thermal retreat paused firing for ${Math.round(v.cooldownTime)}s across ${v.cooldownCount||1} cooldown cycle${(v.cooldownCount||1)===1?'':'s'}. Let systems cool before re-engaging.`);
