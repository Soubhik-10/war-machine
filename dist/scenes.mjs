...
export function workshopScene(machine,{hover=null,selected='cannon',rotation=0,erase=false,grid=true,ghost=true,layer=0,full=true,explode=false,focus=null,phase=0}={}){
 const g=new Geometry();g.material=4;const gap=LAYER_HEIGHT+(explode?.7:0);g.box(0,-.28,0,10.8,.48,10.8,'#222d33');g.bevel(0,-.02,0,10.3,.19,10.3,'#34434a');
...
export function battleScene(battle,{inspect=false,waypoint=true}={}){
 ...
 for(const t of battle.arena.terrain){if(t.type==='vent'&&battle.time%12>=8){const x=wx(t.x+t.w/2),z=wz(t.y+t.h/2);for(let i=0;i<7;i++){const px=x+(noise(i)-.5)*t.w/CELL*.8,pz=z+(noise(i+7)-.5)*t.h/CELL*.8,h=.3+Math.sin(battle.time*8+i)*.15;g.cylinder([px,.12,pz],[px,h+.6,pz],.09,'#f7ac47',7,.9);g.sphere(px,h+.7,pz,.16,'#a28665');}}}
 ...
 for(const v of battle.vehicles){
  ...
  if(v.shield>0){const r=v.radius/CELL+.2;g.ring(pos.x,pos.h+.16,pos.z,r,.05,'#67ccb9',.6);for(let i=0;i<6;i++){const a=i*Math.PI/3;for(let j=0;j<7;j++){const t=j*Math.PI/16,t2=(j+1)*Math.PI/16;g.cylinder([pos.x+Math.cos(a)*Math.cos(t)*r,pos.h+.2+Math.sin(t)*r,pos.z+Math.sin(a)*Math.cos(t)*r],[pos.x+Math.cos(a)*Math.cos(t2)*r,pos.h+.2+Math.sin(t2)*r,pos.z+Math.sin(a)*Math.cos(t2)*r],.012,'#64998f',4,.35);}}}
  if(v.vent>0)for(let i=0;i<7;i++){const age=(battle.time*2+i*.15)%1;g.sphere(pos.x+(noise(i)-.5)*(1+age),pos.h+1+age*1.3,pos.z+(noise(i+3)-.5)*(1+age),.09+age*.15,'#b2c9c7');}
  // Bounded cosmetic dust and damaged-system smoke never consume simulation RNG.
  if(!v.dead&&Math.hypot(v.vx,v.vy)>18&&!v.s.hovering){for(let i=0;i<8;i++){const age=(battle.time*1.8+i*.125)%1;const trail=.15+age*.5;g.sphere(pos.x-v.vx/CELL*trail+(noise(i)-.5)*1.2,pos.h+.08+age*.35,pos.z-v.vy/CELL*trail+(noise(i+8)-.5)*1.2,.035+age*.10,['sand','mud'].includes(v.terrain)?'#99856a':'#5b6668');}}
  for(const m of battle.active(v).filter(m=>m.hp/partSpec(m).hp<.3).slice(0,4)){const p=world(v,m);for(let i=0;i<3;i++){const age=(battle.time*.8+i/3)%1;g.sphere(wx(p.x)+Math.sin(i+age)*age*.25,p.h/CELL+1+age*1.3,wz(p.y)+age*.3,.05+age*.18,'#485153');}}
  if(inspect)for(const m of battle.active(v)){const p=world(v,m),fraction=m.hp/partSpec(m).hp;g.box(wx(p.x),p.h/CELL+1.85,wz(p.y),.72,.07,.1,'#26323a');g.box(wx(p.x)-.36+fraction*.36,p.h/CELL+1.85,wz(p.y),Math.max(.01,fraction*.72),.08,.11,fraction<.35?'#ed9873':color,.5);}
