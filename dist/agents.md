...
3. Inspect valid, issues, stats, environment and advice. Save the returned packed blueprint. This is a share serialization, not proof of victory. Validation is free without login. Contract validation enforces the creator's caps even if your input requests higher ones.
...
See examples/agent-client.mjs and examples/engineer-loop.mjs. The loop is a dry run by default; --enter with --max-entry authorizes exactly one demo attempt. No model, wallet, MPP or npm dependency is required.
