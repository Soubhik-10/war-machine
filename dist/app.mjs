...
<section class="combat-stage" id="combat-stage"><div class="arena-screen"><canvas id="arena-canvas" width="1200" height="800" tabindex="0" aria-label="Deterministic battle arena. Drag to orbit, pinch to zoom. Tap a part to inspect its condition."></canvas>
 ...
 <div class="fight-overlay" id="fight-overlay"><div class="match-card"><small>${challenge ? "FRIEND CHALLENGE" : meta.rank + " SHAKEDOWN"}</small><h2>${esc(enemy.name)}</h2><p>${esc(challenge ? "Break your friend’s machine with a counter built to the locked rules." : meta.hint)}</p><div class="engineering-contract"><span class="auto-indicator"></span><div><strong>AUTOMATIC COMBAT</strong><small>Both machines follow their saved doctrine.</small></div></div><button class="primary" id="start-battle">Start simulation ↗</button><p class="mode-note">100 SECONDS · SAME SEED, SAME RESULT</p></div></div>
 ...
 <div class="observation-deck"><div class="observation-heading"><span class="auto-indicator"></span><strong id="observation-status">SYSTEM STATUS</strong><span id="observation-hint">Doctrine locked at deployment</span></div><div class="weapons-monitor" id="weapons-monitor"></div><div class="system-readout" id="system-readout"></div></div>
...
function scoutRival() {
  ...
  showModal(
    ...
    `<div class="scout-grid"><div><canvas id="scout-canvas" width="640" height="520" aria-label="Rival machine preview. Drag to orbit, pinch or scroll to zoom."></canvas><p class="hint">Drag to orbit · pinch to zoom · front ${frontNames[rival.front || 0]}</p></div><div><span class="eyebrow">${challenge ? "FRIEND CHALLENGE" : "SELECTED RIVAL"}</span><h2>${esc(rival.name)}</h2><p>${s.cost} credits · ${s.mass} t · ${s.parts} parts · ${s.height} levels</p><p>${esc(rival.tactic)} doctrine · ${rival.range} m range · targets ${esc(rival.target)}</p><div class="scout-parts">${[...composition].map(([id, n]) => `<span><b>${n}×</b> ${esc(BY_ID[id].name)}</span>`).join("")}</div></div></div><div class="scout-notes">${engineeringReport(
      ...
      .join(
        "",
      )}</div><div class="modal-footer"><button data-close>Close scout</button><button class="primary" id="scout-refit">Build a counter</button></div>`,
    () => {
    ...
    (officialReceipt
      ...
      : view === "arena"
        ? bountyContext.attemptId
          ...
          : "LOCAL TEST · No entry charge or reward. Return to the bounty to enter an official trial."
