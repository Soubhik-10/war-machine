...
export async function signInWallet(){
 ...
 try{await window.ethereum.request({method:'wallet_switchEthereumChain',params:[{chainId:'0x1079'}]});}catch{throw Error('Switch your wallet to Tempo mainnet (chain 4217), then try again.');}
 [address]=await window.ethereum.request({method:'eth_requestAccounts'});const {message}=await json('/api/auth/challenge',{chainId:4217}),signature=await window.ethereum.request({method:'personal_sign',params:[message,address]});await json('/api/auth/verify',{address,message,signature});paymentClient=null;return address;
}
...
async function payer(){
 ...
 try{await window.ethereum.request({method:'wallet_switchEthereumChain',params:[{chainId:'0x1079'}]});}catch{throw Error('Switch your wallet to Tempo mainnet (chain 4217).');}
 [address]=await window.ethereum.request({method:'eth_requestAccounts'});const client=createClient({account:address,transport:custom(window.ethereum)});
 paymentClient=Mppx.create({polyfill:false,methods:[tempoMethod.charge({account:address,getClient:()=>client,expectedChainId:4217,expectedRecipients:discovery.payments.recipients})],onChallenge:async(challenge,{createCredential})=>{const r=challenge.request,amount=(Number(r.amount)/10**discovery.payments.decimals).toLocaleString(undefined,{maximumFractionDigits:discovery.payments.decimals}),meta=challenge.meta||{};if(!confirm(`Authorize ${amount} ${discovery.payments.currency} on Tempo mainnet?\n\n${r.description||meta.kind||'War Machines payment'}\nRecipient: ${r.recipient}\nNetwork fees are separate and shown by your wallet.`))return;return createCredential();}});return paymentClient;
}
