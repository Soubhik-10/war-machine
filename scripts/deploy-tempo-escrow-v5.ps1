...
$defaultConfig = Join-Path $contractsRoot 'deployments\tempo-mainnet-v5.local.env'
$exampleConfig = Join-Path $contractsRoot 'deployments\tempo-mainnet-v5.env.example'
...
if ($Initialize) {
    ...
    Write-Host "Created public V5 deployment configuration: $ConfigPath"
    Write-Host 'Set the pause guardian, bounded agent relayer, and settlement signer addresses.'
    exit 0
}
if (-not (Test-Path -LiteralPath $forge)) { throw "Tempo Foundry was not found at $forge" }
if (-not (Test-Path -LiteralPath $ConfigPath)) { throw "Missing $ConfigPath. Run: .\scripts\deploy-tempo-escrow-v5.ps1 -Initialize" }
...
Write-Host 'Tempo Mainnet V5 bounty escrow configuration'
...
Write-Host "  relayer:  $relayer (agent operations; keep its access bounded)"
Write-Host "  signer:   $signer (1-of-1 automatic settlement)"
Write-Host "  window:   $window seconds"
Write-Host '  grace:    120 seconds for a verified result relay'
Write-Host '  fee:      fixed at 2.5% of a winning reward to 0xc20131e9132888993de6519D486E5558A5DbCb7A'

$forgeArgs = @(
    'script', 'script/DeployWarMachineBountyEscrowV5.s.sol:DeployWarMachineBountyEscrowV5',
    '--root', $contractsRoot,
