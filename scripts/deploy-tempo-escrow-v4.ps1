...
$defaultConfig = Join-Path $contractsRoot 'deployments\tempo-mainnet-v4.local.env'
$exampleConfig = Join-Path $contractsRoot 'deployments\tempo-mainnet-v4.env.example'
...
if ($Initialize) {
    ...
    Write-Host "Created public V4 deployment configuration: $ConfigPath"
...
if (-not (Test-Path -LiteralPath $ConfigPath)) { throw "Missing $ConfigPath. Run: .\scripts\deploy-tempo-escrow-v4.ps1 -Initialize" }
...
Write-Host 'Tempo Mainnet V4 bounty escrow configuration'
...
$forgeArgs = @(
    'script', 'script/DeployWarMachineBountyEscrowV4.s.sol:DeployWarMachineBountyEscrowV4',
    '--root', $contractsRoot,
