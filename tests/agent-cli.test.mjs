...
test('external CLI validates, bookmarks, submits and retries one verified climate trial',async t=>{
 ...
 const bounty=app.store.create(creator.me.id,{title:'CLI climate test',entry:0,reward:5,hours:0,listed:false,blueprint:packChallenge(PRESETS[0],'sunscar',0)},randomUUID());
 ...
 const attempt=await run('submit',bounty.id,'blueprint.json','0'),files=await readdir(join(dir,'work'));assert.equal(files.length,1);
