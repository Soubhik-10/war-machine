...
async function main(){
 ...
 const report={base,bounty:bounty.id,engineHash:hash,terms:{entry:bounty.entry,reward:bounty.reward,platformFeeBps:bounty.platformFeeBps,platformFee:bounty.platformFee,payout:bounty.payout,netIfWin:bounty.netIfWin},officialAttempt:false,selected:{...selected,heldOut:held},ranked,rejected,note:'Held-out seeds were not used to select a build. Official seeds remain unknown; practice does not guarantee a win.'};
 ...
 console.log(JSON.stringify({output:out,selected:selected.name,heldOutWins:held.wins,heldOutMatches:held.matches,demoCreditsSpent:0,terms:report.terms,disclosure:'Platform fee is deducted from gross winnings; entry is separate.'},null,2));
 if(opts['--enter']){const fresh=await request('/api/bounties/'+bounty.id),maxEntry=Number(opts['--max-entry']),maxPlatformFeeBps=Number(opts['--max-platform-fee-bps']);if(fresh.status!=='open'||!fresh.compatible||fresh.entry>maxEntry||fresh.platformFeeBps>maxPlatformFeeBps)throw Error('Contract is no longer eligible or exceeds your explicit fee cap. No attempt sent.');const attempt=await durablePost('/api/bounties/'+bounty.id+'/attempts',{blueprint:selected.blueprint,maxEntry,maxPlatformFeeBps});await writeFile(join(out,'attempt.json'),JSON.stringify(attempt,null,2));report.officialAttempt=attempt.id;await writeFile(join(out,'report.json'),JSON.stringify(report,null,2));console.log(JSON.stringify({officialAttempt:attempt.id,status:attempt.status,poll:'/api/attempts/'+attempt.id,share:base+'/#bounty='+bounty.id},null,2));}
}
