...
///      create/entry calls are bounded by the configured relayer; direct users keep
///      the direct creator and challenger methods. V5 is unchanged for legacy data.
contract WarMachineBountyEscrowV6 {
    ...
    event AttemptEntered(
        uint256 indexed bountyId, uint64 indexed attemptNonce, address indexed challenger, uint64 attemptDeadline
    );
    ...
    uint256 public reservedEntries;
    ...
    constructor(
        ...
        if (address(token_) == address(0) || pauseGuardian_ == address(0) || agentRelayer_ == address(0)) revert InvalidAddress();
        ...
        if (
            count < MIN_SETTLEMENT_QUORUM || count > MAX_SETTLEMENT_SIGNERS || quorum_ < MIN_SETTLEMENT_QUORUM
                || quorum_ > count
    ...
    function createBountyFor(address creator, bytes32 termsHash, uint128 reward, uint128 entry, uint64 expiresAt)
        external
        onlyAgentRelayer
        nonReentrant
        returns (uint256 bountyId)
    ...
    function enterBountyFor(address challenger, uint256 bountyId) external onlyAgentRelayer nonReentrant {
        if (challenger == address(0)) revert InvalidAddress();
        _enter(bountyId, challenger, agentRelayer);
    }

    function settleAttempt(Settlement calldata settlement, bytes[] calldata signatures) external nonReentrant {
        Bounty storage bounty = bounties[settlement.bountyId];
        if (bounty.status != BountyStatus.Active || settlement.attemptNonce != bounty.attemptNonce) {
    ...
    function forfeitTimedOutAttempt(uint256 bountyId) external nonReentrant {
        _refundTimedOutAttempt(bountyId);
    }
    ...
    function _refundTimedOutAttempt(uint256 bountyId) private {
        Bounty storage bounty = bounties[bountyId];
        if (bounty.status != BountyStatus.Active) revert InvalidStatus();
        if (block.timestamp < bounty.attemptDeadline + SETTLEMENT_GRACE) {
            revert AttemptNotTimedOut();
        }
        uint128 entry = bounty.entry;
        address challenger = bounty.challenger;
        address creator = bounty.creator;
        uint64 nonce = bounty.attemptNonce;
        bounty.challenger = address(0);
        bounty.attemptDeadline = 0;
        bounty.status = BountyStatus.Open;
        _releaseEntry(entry);
        if (entry != 0) _pushExact(challenger, entry);
        emit TimedOutAttemptRefunded(bountyId, nonce, challenger, creator, entry);
    }
    ...
    function _create(address creator, bytes32 termsHash, uint128 reward, uint128 entry, uint64 expiresAt)
        private
        returns (uint256 id)
    {
        id = nextBountyId++;
        bounties[id] = Bounty(creator, address(0), reward, entry, expiresAt, 0, 0, BountyStatus.Open, termsHash);
        reservedRewards += reward;
        emit BountyCreated(id, creator, reward, entry, expiresAt, termsHash);
    }
    ...
    function _enter(uint256 bountyId, address challenger, address payer) private {
        ...
        if (bounty.entry != 0) _pullExact(payer, bounty.entry);
    ...
    function _releaseEntry(uint128 amount) private {
        reservedEntries -= amount;
    }
    ...
    function _verifyQuorum(Settlement calldata settlement, bytes[] calldata signatures) private view {
