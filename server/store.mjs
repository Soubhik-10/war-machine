...
export class Store {
 constructor(path=':memory:',{now=Date.now,seed=true}={}){
  ...
  if(seed&&!this.db.prepare('SELECT 1 FROM meta WHERE key=?').get('starter:'+ENGINE_HASH))this.tx(()=>{
   ...
   for(const [i,arena,title] of [[0,'foundry','First contract · Break the Marauder'],[6,'reservoir','Steam test · Firefly'],[1,'quarry','Open a firing lane · Longbow'],[5,'badlands','Crack the Ironclad']])this.create(owner,{title,blueprint:packChallenge(PRESETS[i],arena,0),entry:10,reward:100,hours:168,listed:true},id(),true);
 ...
 credit(account,amount,kind,ref){check(Number.isSafeInteger(amount),'Invalid ledger amount.',500);const exists=this.db.prepare('SELECT amount FROM ledger WHERE account=? AND kind=? AND ref=?').get(account,kind,ref);if(exists){check(exists.amount===amount,'Ledger conflict.',500);return;}const changed=this.db.prepare('UPDATE accounts SET balance=balance+? WHERE id=? AND balance+?>=0').run(amount,account,amount);check(changed.changes===1,'Not enough demo credits.',409);this.db.prepare('INSERT INTO ledger VALUES (?,?,?,?,?,?)').run(id(),account,amount,kind,ref,this.now());}
 ledger(account){return this.db.prepare('SELECT amount,kind,ref,created FROM ledger WHERE account=? ORDER BY created DESC,rowid DESC LIMIT 100').all(account);}
 ...
 create(account,body,key,nested=false){const fn=()=>{
  fields(body,['title','blueprint','entry','reward','hours','listed']);const old=this.prior(account,key,'create',body);if(old)return this.bounty(old.ref);
  ...
  this.db.prepare('INSERT INTO bounties VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)').run(bounty,account,title,json(blueprint),fee,reward,'open',+body.listed,now,hours?now+hours*3600000:0,json(versions),null,null,reward);
 ...
 accept(account,bounty,body,key){return this.tx(()=>{
  fields(body,['blueprint','maxEntry']);const old=this.prior(account,key,'enter:'+bounty,body);if(old)return this.attempt(old.ref,account);
 ...
 attempt(id,account){const a=this.db.prepare('SELECT * FROM attempts WHERE id=?').get(id);check(a,'Attempt not found.',404);const b=this.rawBounty(a.bounty),done=['settled','refunded'].includes(a.status);check(done||account===a.account||account===b.owner,'This attempt is still private while running.',403);return {id:a.id,bounty:a.bounty,account:a.account,status:a.status,created:a.created,updated:a.updated,result:a.result?parse(a.result):null,error:a.error,...(done?{replay:{challenger:parse(a.blueprint),defender:parse(b.blueprint),arena:parse(b.blueprint).a,seed:a.seed,swapSpawns:!!(a.seed&1),versions:parse(b.version)}}:{})};}
 ...
 finish(job,result,error){return this.tx(()=>{
  ...
  const receipt={...result,outcome:result.winner===0?'win':result.winner===1?'loss':'draw',entry:b.fee,reward:result.winner===0?b.reward:0,net:result.winner===0?b.reward-b.fee:-b.fee,verifiedAt:this.now()};
  this.db.prepare("UPDATE attempts SET status='settled',result=?,updated=?,lease=NULL WHERE id=?").run(json(receipt),this.now(),a.id);
  if(result.winner===0){this.credit(a.account,b.reserve,'reward',a.id);this.db.prepare("UPDATE bounties SET status='claimed',reserve=0,active=NULL,winning=? WHERE id=?").run(a.id,b.id);}else this.reopen(b);return true;
 });}
 ...
 catalog(){return {mode:'demo',apiVersion:'2',discovery:'/.well-known/war-machines.json',openapi:'/api/openapi.json',terrainInfo:TERRAIN_INFO,economics:{maxInteger:MAX_CREDITS,entryMin:0,rewardMin:0,rewardMustExceedEntry:false,personalCapsDefault:null,hours:{min:0,max:8760,zero:'No expiry'}},guest:['build','save local blueprints','share machines','browse','validate','practice'],loginRequired:['create bounty','enter official trial','save bounty','account history'],walletAuth:'TODO',mpp:'TODO',versions,startingCredits:1000,parts:PARTS,arenas:ARENAS,defaultRules:DEFAULT_RULES,examples:PRESETS.map(m=>packChallenge(m,'foundry',0)),rules:{oneActiveAttempt:true,combat:'auto',timeLimitSeconds:100,drawIntegrityThreshold:.025,entryRefund:'Technical failure only',spending:'Entry cap and daily UTC entry budget; funding reserves are separate',payments:'TODO — no wallets, MPP, Tempo or cash value'}};}
}
