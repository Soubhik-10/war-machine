...
export function discovery(config={mode:'demo',paymentsEnabled:false}){const paid=config.paymentsEnabled;return {
 ...
 authentication:{guest:['catalog','contracts','validation','practice'],account:['create/cancel bounty','official entry','save bounty','history'],scheme:'Bearer or Secure HttpOnly session cookie',ownerOnly:['set spending caps','issue/revoke agent keys'],wallet:paid?{challenge:'/api/auth/challenge',verify:'/api/auth/verify',session:'/api/auth/session',logout:'/api/auth/logout',passkeyBase:'/api/auth/passkey',chainId:config.network.chainId}:'disabled'},
