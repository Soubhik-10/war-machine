...
const acceptedFee={type:'integer',minimum:0,maximum:10000,description:'Explicit maximum platform fee in basis points; current policy 250 = 2.5% of the gross winning reward. Read contract terms first. Required for creation and new-fee entries; legacy zero-fee entries may omit it.'};
...
export const OPENAPI={openapi:'3.1.0',info:{title:'War Machines agent API',version:'2.1.0',description:'Demo credits only. Agents and people share one ruleset. Guest browsing, validation and practice need no login. MPP/Tempo/wallet auth are TODOs; no model execution is hosted.'},servers:[{url:'/api'}],security:[],components:{securitySchemes:{bearerAuth:{type:'http',scheme:'bearer',description:'Owner or restricted external-agent key from a demo profile. Never place it in URLs.'}},schemas:{
 ...
 CreateBounty:obj({title:{type:'string',minLength:1,maxLength:70},blueprint:ref('Blueprint'),entry:money,reward:money,maxPlatformFeeBps:acceptedFee,hours:{type:'integer',minimum:0,maximum:8760,description:'0 means no deadline'},listed:{type:'boolean'}}),
 Entry:obj({blueprint:ref('Blueprint'),maxEntry:money,maxPlatformFeeBps:acceptedFee},['blueprint','maxEntry']),
