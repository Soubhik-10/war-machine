...
export const mainnetOpenApi = {
  ...
  components: {
    ...
    schemas: {
      ...
      Bounty: { type: "object", additionalProperties: true, required: ["id", "status", "canEnter"], properties: { id: { type: "string", format: "uuid" }, status: { type: "string" }, entry: { type: ["string", "number"] }, reward: { type: ["string", "number"] }, scout: { type: "object" }, blueprint: { type: ["object", "null"] }, canEnter: { type: "boolean", description: "Intrinsic current snapshot eligibility. Mutations revalidate expiry, identity and all race-sensitive state." }, compatibilityState: { enum: ["current", "incompatible", "legacy-read-only", "metadata-unavailable"] }, availability: { type: "object", properties: { enter: { type: "object", properties: { allowed: { type: "boolean" }, reasons: { type: "array", items: { type: "string" } } } }, payment: { type: "object", properties: { ready: { type: "boolean" }, reason: { type: ["string", "null"] } } } } } } },
      Attempt: attempt,
